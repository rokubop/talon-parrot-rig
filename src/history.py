"""The two histories the palate picker offers, kept apart on purpose.
...
from ..parrot_rig_settings import PALATE_RECENT_COUNT, PALATE_HISTORY_SKIP
...
def parrot_history_record(mode: str, key: str, label: str):
    ...
    del _parrot[PALATE_RECENT_COUNT:]
...
def _is_skipped(label: str) -> bool:
    lowered = label.lower()
    return any(word in lowered for word in PALATE_HISTORY_SKIP)


def parrot_history(limit: int = None) -> list:
    return _parrot[:limit or PALATE_RECENT_COUNT]
...
def voice_history(limit: int = None) -> list:
    ...
    limit = limit or PALATE_RECENT_COUNT
    phrases = []
    index = 0
