"""What the palate noise does. One binding, three kinds of thing it can be.

preset - an entry in utility_maps, the eight mouse actions palate always had
...
def palate_set_preset(key: str):
    global _binding
    if key in _presets():
        _binding = {"kind": PRESET, "key": key}


def palate_set_phrase(phrase: str):
    global _binding
...
def palate_set_parrot(mode: str, key: str, label: str):
    global _binding
    _binding = {"kind": PARROT, "mode": mode, "key": key, "label": label}


def palate_apply(data: dict):
...
def palate_is(kind: str, ident=None) -> bool:
    """Whether the picker should show a tile as the current binding."""
    binding = palate_binding()
...
def palate_run():
    """Fire whatever palate is bound to."""
    binding = palate_binding()
    if binding["kind"] == PRESET:
        entry = _presets().get(binding["key"])
        if entry:
