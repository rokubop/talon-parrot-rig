...
| `shush` | scroll / boost | Scroll up, boost in move mode. Ramps in over a second, for covering a screen. Locks the heading it fired on |
...
| `palate` | utility 1 / fast boost | App picker by default, `tut palate` rebinds it. While moving it is the immediate boost instead, linear both ways, for a gap you can already see. Same heading lock |
