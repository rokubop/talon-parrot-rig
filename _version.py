...
This file provides:
- Package version checking via parrot_rig_version() action
...
try:
    ...
    print(f"ERROR: talon-parrot-rig failed to load version from manifest.json: {e}")
    _VERSION = (0, 0, 0)

@mod.action_class
class Actions:
    def parrot_rig_version() -> tuple[int, int, int]:
        """
        Returns the package version as (major, minor, patch).

        Usage: actions.user.parrot_rig_version() >= (1, 2, 0)
        """
        return _VERSION
